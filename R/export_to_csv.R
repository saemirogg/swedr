...
convertToCSV <- function(path, overwrite=FALSE, max_rows=1024){
  ...
  # Use max_rows to break up into chunks to consume less memory.
  # Set overwrite to TRUE if you want to overwrite existing .csv files.
  #
  # Example usage:
  # path <- "/path/to/data/"
  ...
  # for (f in files) {
  #     if(grepl(".RData", f)) {
  #         convertToCSV(paste(path, f, sep=""), overwrite=TRUE, max_rows=2^16)
  #     }
  # }
